<x-app-layout>
  <div class="bg-[url('http://localhost:8000/img/pokemon-halloween4.jpg')] bg-cover bg-center h-screen w-screen flex items-center justify-center">
    ...
    <form class="w-80 h-2/4 relative right-10 top-4 bottom-14 bg-shadow-md rounded px-12 pt-12 pb-20 mb-4 bg-cover bg-[url('http://localhost:8000/img/bg-1.jpeg')]" action="{{ url('owner') }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="flex items-center justify-center">
        <a href="{{ url('owner/') }}" class="bg-purple-700 hover:bg-purple-900 text-white font-bold py-2 px-6 rounded mt-3 focus:outline-none focus:shadow-outline focus:ring focus:ring-purple-300 active:bg-purple-700">
          See your Owners
        </a>
      </div>
